...
def repeatlocatorLongRead(long_read_file, NLRmotif=None):
    ...
    if NLRmotif:
        read_long_reads = list(
            filter(None, [x.strip() for x in open(long_read_file).readlines()])
        )
        ...
        long_read_dataframe = pd.DataFrame(
            [(i, j) for i, j in zip(ids, sequences)]
        ).rename(columns={0: "ids", 1: "sequences"})
        long_read_dataframe["repeat_locator"] = (
            long_read_dataframe["sequences"]
            .apply(lambda n: str(n))
            .apply(lambda n: re.findall(r"[NLRmotif]", n))
        )
        repeats = list(
            set(
                [
                    j
                    for i in (long_read_dataframe["repeat_locator"].to_list())
                    for j in i
                ]
            )
        )
        long_read_dataframe["fraction_length"] = long_read_dataframe["sequences"].apply(
            lambda n: [
        ...
        fraction_length_plot = list(
            map(lambda n: sum([i[1] - i[0] for i in n]), fractions)
        )
        long_read_dataframe["fraction_length_coverage"] = pd.DataFrame(
            [
                sequences_length[i] / fraction_length_plot[i]
